...
class _SlideshowState extends State<Slideshow> {
  ...
  void initState() {
    super.initState();
    shiftTimer = Timer.periodic(updateDuration, _shiftMedias);
                ...
                if (dir != null) {
                  ...
                  if (file.fileSize > 1024000 &&
                    ...
                    await downloadFile(
                      ...
                      // Add media file info on local db
                      var media = MyMediaFile(
                        fileType: file.fileType,
                        fileSize: 0,
                        isReminder: false,
                      ...
                      var sortedList =
                          slideList.where((s) => (s.id.contains(file.id)));
  ...
  Future<void> downloadFile(String fileUrl, String filePath) async {
    Dio dio = Dio();
      ...
      dio.download(
        ...
        onReceiveProgress: (count, total) {
          ...
          if (currentStep == 100) {
            ...
            // setState(() {
            // });
            // Restart Downlaod session and download again or done with progress
            // db.getAllFiles('SkgTAJzMFwPpKJBrZg3czljggdX2').then((allFiles) {
            _fetchFiles(allFilesRestored);
          }
        },
      );
    } catch (e) {
  ...
  _shiftMedias(Timer timer) async {
    if (mounted) {
